...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    switch(size) {
      case "1":
        document.getElementById("pizzaSize").textContext = "Small";
        break;
      case "2":
        document.getElementById("pizzaSize").textContext = "Medium";
        break;
      case "3":
        document.getElementById("pizzaSize").textContext = "Large";
        break;
      default:
...
// Create document fragment so the subsequent for loop does not
// cause page reflow by touching the DOM on each iteration.
var pizzaFragment = document.createDocumentFragment();

// This for loop creates and appends all of the pizzas to the
// above fragment when the page loads.
for (var i = 2; i < 100; i++) {
  pizzaFragment.appendChild(pizzaElementGenerator(i));
}

// The fragment is then appended to the DOM.
...
// Prevents updatePositions from firing on every scroll event.
// Instead it just fires once per frame.
function throttleUpdatePositions() {
  if(!animating) {
    animating = true;
...
window.addEventListener('scroll', throttleUpdatePositions);
...
function loadPizzas() {
  ...
  pizzaStats = calculatePizzas(),
  pizzaColumns = pizzaStats[0],
  pizzaAmount = pizzaStats[1];

  // If window is resized after initial load then all currently visible
  // pizzas are deleted before pizza columns/amount are updated.
  while (movingPizzas1.firstChild) {
    movingPizzas1.removeChild(movingPizzas1.firstChild);
  }
  ...
  window.pizzaStats = pizzaStats;
  updatePositions();
}

// Returns an array, the first index of which indicates how many moving
// columns of pizzas there are, the second index indicates how many
// pizzas are loaded on the page. Returned values depend on the
// width/height of the user's window, ensuring that the browser only
// loads the minimum amount of pizzas necessary to fill the viewport.
function calculatePizzas() {
  ...
  if(width <= 414) {
    ...
    pizzaStats[0] = 4;
    pizzaStats[1] = 24;
  } else if(width > 768 && width <= 1024 && height <= 1366) {
    pizzaStats[0] = 6;
    pizzaStats[1] = 40;
...
// Recalculates the pizza columns/amount when user resizes window.
window.addEventListener('resize', loadPizzas);
